...
class Reports_Controller extends Admin_Controller {
	...
	public function edit($id = FALSE, $saved = FALSE)
	{
		...
		// Are we creating this report from a OC feed?
		if ( isset($_GET['OCid']) AND intval($_GET['OCid']) > 0 )
		{
			$OCfeed_item_id = intval($_GET['OCid']);
			$OCfeed_item = ORM::factory('OCfeed_item', $feed_item_id);
			...
			if ($OCfeed_item->loaded)
			{
				...
				// $form['indicent_CUP'] = $OCfeed_item->item_CUP;
			...
			if (Incident_Model::is_valid_incident($id, FALSE))
			{
				...
				if ($incident->loaded == TRUE)
				{
					...
					$incident_arr = array(
						...
						'incident_CUP' => $incident->incident_CUP,
